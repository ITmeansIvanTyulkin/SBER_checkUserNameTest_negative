...
public class DeleteProcessTemplate {
    ...
    private final By listOfWebElementsOfTheTemplates = By.xpath("//div[@class = 'TemplatesPanel_content__1YpsI']");

    // Методы.
    private final WebDriver driver;

    public DeleteProcessTemplate(WebDriver driver) {
        this.driver = driver;
    }
    ...
    @Step("Проверяю, есть ли тестовый шаблон в списке всех шаблонов на странице.")
    public void checkIfTheTestTemplateThatHasBeenDeletedIsStillOnThePage() {
        List<WebElement> elements = driver.findElements(listOfWebElementsOfTheTemplates);
        if (elements.stream()
                .map(WebElement :: getText)
                .anyMatch(actualValue -> actualValue.equals(TEMPLATE_NAME))) {
            throw new RuntimeException("The template hasn't been deleted!");
        }
    }
    ...
    @Step("Нажатие на кнопку удаления шаблона (тестовых данных) без последующего подтверждения удаления шаблона.")
    ...
    @Step("Нажимаю на кнопку 'Да' в окне, подтверждающем удаление шаблона.")
    public void confirmButtonYesClick() {
        driver.findElement(confirmButtonToDeleteTheTestTemplate).isEnabled();
        driver.findElement(confirmButtonToDeleteTheTestTemplate).click();
    }
